...
- **Android crash when a publication closes during a page load**: closing a publication while the image navigator was still loading a page killed the app with `java.lang.IllegalStateException: zip file closed`. Closing the publication closes the backing `ZipFile`, but cancellation is cooperative, so a read already inside `withContext(Dispatchers.IO)` runs on and reaches the closed file. readium 3.1.2 catches only `ZipException` and `IOException` there, so that throw escapes the `Try<ByteArray, ReadError>` its `read()` declares, and it lands in a readium fragment whose coroutine context carries no `Job`: no parent, no handler, and Android takes the process down. A read against a closed container now reports `ReadError.Access`, which is what readium's own signature promises. The guard goes on at the container boundary, so it covers CBZ, DIVINA, EPUB and PDF. It matches only `IllegalStateException` carrying `ZipFile`'s own "zip file closed" text, and re-throws `CancellationException` ahead of that check, since `CancellationException` subclasses `IllegalStateException` and swallowing it would break the cancellation contract the method channel relies on. The race is upstream and still there; this stops it killing the host app.
...
- Android JVM (Robolectric): the closed-container guard reports a read against a closed `ZipFile` as `ReadError.Access`, lets `CancellationException` through untouched, leaves every other runtime failure loud, and delegates `close()` and `sourceUrl`. One test builds the guard the way the open path builds it, with `TransformingResource` in between, since the composed expression is what has to hold rather than the guard on its own.
- The Android integration job now records `logcat` and a per-test event stream and uploads both when it fails. It had been dying part way through roughly one run in fifteen since June with nothing kept from the run, so none of the first three occurrences could be read; the fourth was diagnosable in minutes. The capture lives in `.github/scripts/android_integration_tests.sh` rather than inline in the workflow, because `reactivecircus/android-emulator-runner` splits its `script:` input on newlines and runs each line in a separate `sh -c`. `android_integration_tests_test.sh` fails if the workflow turns that input back into a shell block.
