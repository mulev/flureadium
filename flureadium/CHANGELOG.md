...
- `docs/05-testing/integration-tests.md` lists working name resolution on the Android device as a prerequisite, next to the TTS engine: why the emulator can end up holding a resolver it has no route to, why a failed `ping` says nothing about it, and the `-dns-server` fix.
...
- A failed open leaves a trace a test can read. Each of the fifteen open paths carried its own `catch (e) { debugPrint(...) }`, so an open that threw printed a line to the log and changed nothing on screen — a test could only notice by looking for a reader that never appeared, and a reader left over from an earlier test hid even that. One `_runOpen` helper now wraps all fifteen and records the failure into a keyed `open-error` latch, which it clears again whenever a path succeeds so a stale error cannot fail a later assertion. `_runOpen` does that clearing rather than `_resetPublicationLatches`, because **Load Only** loads without opening and never reaches that reset — a failed open followed by a good load used to leave the error latched, which is the false failure the latch exists to prevent. `_toggleAudio` keeps its own catch: it raises a SnackBar the widget suite asserts on. A second latch, `publication-identifier`, names the publication currently open, because the WebPub manifest and the bundled EPUB fixture are both titled "Moby-Dick" and only the identifier tells them apart. `integration_test/helpers/publication_latch.dart` reads all three.

### Testing

- The WebPub integration test can fail now. `webpub_test.dart` tapped **Open WebPub** and then asserted that a `ReadiumReaderWidget` was on screen, which the app's launch-time EPUB open had already made true — the condition held on the first pump and the test ended long before the remote open could have finished. The open failed on three consecutive runs and all three passed. The test now samples `open-generation` before the tap, waits for it to move, and asserts that `open-error` is empty and that `publication-identifier` is `urn:isbn:9780000000001`. The identifier rather than the title, because the remote manifest and the bundled fixture are both called "Moby-Dick" and only the identifier separates them. Proved by pointing the opener at a host under `.invalid` and watching Android and iOS both fail with the open's own error, then putting the real URL back.
- `run_integration_tests.sh` checks name resolution on the Android device before the Android leg. When the device cannot open a connection to `readium.org:443` by name, the Android leg does not start — nothing is installed, the `-dns-server` remedy is printed, and the leg is reported NOT RUN — instead of running a suite whose `network`-tagged tests are all certain to fail. It stops that leg and not the process: the iOS and Web legs still run and the summary still names what did not, because a broken resolver on one device is no reason to drop the coverage of two others. A dead resolver used to surface as a red integration test, and once as a filed plugin bug. The probe is a TCP connect by name over `adb shell`, never a `ping`: the emulator's virtual router forwards TCP and UDP but may not forward ICMP, so a failed ping is the normal state of a healthy emulator, and reading one as a dead network is what caused the misdiagnosis. It runs on the device rather than the host, because on the machine where this was measured the host resolved names fine and the guest resolved nothing. Unlike the TTS pinning beside it, the check is not best-effort — a probe that cannot run at all fails the run too, at the end, with the suite named. `--help` also prints its whole header again: `usage()` read a hard-coded line range that stopped on the bare "Tags:" heading and dropped everything after it.
