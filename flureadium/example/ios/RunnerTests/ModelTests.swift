...
final class ImageReaderNavigationStateTests: XCTestCase {
    ...
    func testDefaultsMatchImageReaderExpectations() {
        let state = ImageReaderNavigationState()
    ...
    func testApplyUpdatesFlagsAndClampsThreshold() {
        var state = ImageReaderNavigationState()
    ...
    func testClampCapsThresholdAtMaximum() {
        XCTAssertEqual(ImageReaderNavigationState.clamp(240.0), 120.0)
    }

    func testConfigureSetsHandlersWhenEnabled() {
        let view = EdgeTapInterceptView(frame: CGRect(x: 0, y: 0, width: 100, height: 100))
        var leftCalls = 0
        var rightCalls = 0
        var swipeLeftCalls = 0
        var swipeRightCalls = 0

        var state = ImageReaderNavigationState()
        state.edgeTapAreaPoints = 72.0
        state.configure(
            edgeTapView: view,
            onLeftEdgeTap: { leftCalls += 1 },
            onRightEdgeTap: { rightCalls += 1 },
            onSwipeLeft: { swipeLeftCalls += 1 },
            onSwipeRight: { swipeRightCalls += 1 }
        )
        ...
        XCTAssertEqual(leftCalls, 1)
        XCTAssertEqual(rightCalls, 1)
        XCTAssertEqual(swipeLeftCalls, 1)
        XCTAssertEqual(swipeRightCalls, 1)
    }

    func testConfigureClearsEdgeHandlersWhenDisabled() {
        let view = EdgeTapInterceptView(frame: CGRect(x: 0, y: 0, width: 100, height: 100))
        view.onLeftEdgeTap = {}
        view.onRightEdgeTap = {}
    ...
    func testConfigureClearsSwipeHandlersWhenDisabled() {
        let view = EdgeTapInterceptView(frame: CGRect(x: 0, y: 0, width: 100, height: 100))
        view.onSwipeLeft = {}
        view.onSwipeRight = {}
