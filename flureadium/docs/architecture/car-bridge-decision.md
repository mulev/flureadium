...
- **Invalidation (outbound refresh)** = `refreshCarContent()` on the `Flureadium` facade → platform. The transport otherwise stays native-asks / Dart-answers (`CarContentTransport`), so a live surface never learns the library changed. This one fire-and-forget outbound nudge closes that gap: the host fires it on every browsable-set mutation and native re-asks. It maps to iOS `updateSections(_:)` on the retained `CPListTemplate`s (Phase 3) and Android `notifyChildrenChanged` on the subscribed browse parents (Phase 5). Variant (a) reuses the host's Dart re-read on each `children` call — there is no native mirror to invalidate, unlike (b), which needed an invalidation hook feeding a duplicate store.
...
- **iOS refresh (Phase 3)** Dart `refreshCarContent` makes `FlureadiumPlugin` post `Notification.Name.flureadiumCarContentShouldRefresh` on the app engine. The CarPlay scene (Phase 7, same process) observes it and calls `CarTemplateRenderer.refresh()`, which re-fetches each retained root tab's children and repaints its `CPListTemplate` through `updateSections`. Pushed detail lists are left alone; they re-fetch on the next navigation, so a live browse is never redrawn under the user. The repaint is idempotent: a tab whose refreshed children no longer include the Siri marker also drops its assistant cell.
...
- **Android refresh (Phase 5)** Dart `refreshCarContent` reaches `PluginMediaService.instance.refreshBrowse()` through the `/main` `PublicationMethodCallHandler`. `refreshBrowse` posts onto the main looper (the route can arrive on any thread, while the method-channel source and `MediaLibrarySession` are main-thread-only) and calls `PluginLibrarySessionCallback.notifySubscribedParents`. That re-notifies each subscribed `(controller, parentId, params)` record via the per-controller `notifyChildrenChanged` overload, so every browser keeps its own `LibraryParams`; the announced child count comes from `onGetChildren` itself, so it matches a re-query exactly (the empty-state status row is counted, `siri` markers are not). Android Auto then re-queries only the parents it is browsing. A refresh queued as the service is torn down is dropped by an instance-identity check, so it never touches a released session.
